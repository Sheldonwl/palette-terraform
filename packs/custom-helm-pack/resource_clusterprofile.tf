...
data "spectrocloud_registry" "helm-repo" {
  name = "Bitnami"
}

data "spectrocloud_pack" "dokuwiki" {
  name = "dokuwiki"
  registry_uid = data.spectrocloud_registry.helm-repo.id
  version  = "14.3.2"
}

data "spectrocloud_pack" "spectrok8sdash" {
...
pack {
    name   = "dokuwiki"
    tag    = data.spectrocloud_pack.dokuwiki.version
    uid    = data.spectrocloud_pack.dokuwiki.id
    values = <<-EOT
pack:
    namespace: "dokuwiki"
...
image:
  ...
  tag: 20230404.1.0-debian-11-r102
  digest: ""
...
## @param resources.limits [object] The requested limits for the container
##
resources:
  requests:
    memory: 512Mi
...
pack {
    name   = "spectro-k8s-dashboard"
    tag    = data.spectrocloud_pack.spectrok8sdash.version
    uid    = data.spectrocloud_pack.spectrok8sdash.id
    values = <<-EOT
    pack:
...
charts:
  kubernetes-dashboard:
    ...
    podDisruptionBudget:
      ...
      minAvailable: ## Maximum unavailable instances; ignored if there is no PodDisruptionBudget
